...
uint16_t DFRobot_SHT20::readValue(byte cmd)
{
    _pWire->beginTransmission(_addr);
    _pWire->write(cmd);
    if(_pWire->endTransmission() != 0){   // Used Wire.endTransmission() to end a slave transmission started by beginTransmission() and arranged by write().
...
void flushI2CBuffer(TwoWire *pWire) {
  while (pWire->available()) {  // While there is data in the buffer
    pWire->read();  // Read and discard a byte
  }
}
